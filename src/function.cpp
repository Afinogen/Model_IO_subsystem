...
void InitButton()
{
    pButtonStartPause = new Button(pFormSMO->GetHInstance(),
            pFormSMO->GetHWND(), "Старт");
    pButtonStartPause->CreateButton(20, 50, 130, 35);

    pButtonStop = new Button(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "Стоп");
...
void InitLabel()
{
    pLabelCountQuery = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "Количество заявок");
    pLabelCountQuery->CreateLabel(10, 10, 180, 20);

    pLabelCountQueryTrun = new Label(pFormSMO->GetHInstance(),
            pFormSMO->GetHWND(), "Заявок в очереди: 0");
    pLabelCountQueryTrun->CreateLabel(10, 100, 280, 20);

    pLabelSK1 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "СК 1: свободен");
    pLabelSK1->CreateLabel(10, 130, 180, 20);

    pLabelSK2 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "СК 2: свободен");
    pLabelSK2->CreateLabel(230, 130, 180, 20);

    pLabelDescNMD1 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "НМД1");
    pLabelDescNMD1->CreateLabel(10, 170, 60, 20);

    pLabelDescNMD2 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "НМД2");
    pLabelDescNMD2->CreateLabel(80, 170, 60, 20);

    pLabelDescNMD3 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "НМД3");
    pLabelDescNMD3->CreateLabel(160, 170, 60, 20);

    pLabelDescNMD4 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "НМД4");
    pLabelDescNMD4->CreateLabel(240, 170, 60, 20);

    pLabelDescNMD5 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "НМД5");
    pLabelDescNMD5->CreateLabel(320, 170, 60, 20);

    pLabelDescNMD6 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "НМД6");
    pLabelDescNMD6->CreateLabel(400, 170, 60, 20);

    pLabelTimeNMD1 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "0");
    pLabelTimeNMD1->CreateLabel(10, 190, 60, 20);

    pLabelTimeNMD2 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "0");
    pLabelTimeNMD2->CreateLabel(80, 190, 60, 20);

    pLabelTimeNMD3 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "0");
    pLabelTimeNMD3->CreateLabel(160, 190, 60, 20);

    pLabelTimeNMD4 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "0");
    pLabelTimeNMD4->CreateLabel(240, 190, 60, 20);

    pLabelTimeNMD5 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "0");
    pLabelTimeNMD5->CreateLabel(320, 190, 60, 20);

    pLabelTimeNMD6 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "0");
    pLabelTimeNMD6->CreateLabel(400, 190, 60, 20);

    pLabelCollectionClient = new Label(pFormSMO->GetHInstance(),
            pFormSMO->GetHWND(), "Обработано заявок: 0");
    pLabelCollectionClient->CreateLabel(10, 230, 270, 20);

    pLabelTimeWork = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "Общее время работы: 0");
    pLabelTimeWork->CreateLabel(10, 260, 270, 20);

    pLabelLoadSK1 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "Загрузка СК1: 0%");
    pLabelLoadSK1->CreateLabel(10, 290, 270, 20);

    pLabelLoadSK2 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "Загрузка СК2: 0%");
    pLabelLoadSK2->CreateLabel(10, 320, 270, 20);

    plabelLoadNMD1 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "Загрузка НМД1 0%");
    plabelLoadNMD1->CreateLabel(10, 360, 180, 20);

    plabelLoadNMD2 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "Загрузка НМД2 0%");
    plabelLoadNMD2->CreateLabel(10, 390, 180, 20);

    plabelLoadNMD3 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "Загрузка НМД3 0%");
    plabelLoadNMD3->CreateLabel(10, 420, 180, 20);

    plabelLoadNMD4 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "Загрузка НМД4 0%");
    plabelLoadNMD4->CreateLabel(10, 450, 180, 20);

    plabelLoadNMD5 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "Загрузка НМД5 0%");
    plabelLoadNMD5->CreateLabel(10, 480, 180, 20);

    plabelLoadNMD6 = new Label(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "Загрузка НМД6 0%");
    plabelLoadNMD6->CreateLabel(10, 510, 180, 20);

    pLabelMidleTimeClinet = new Label(pFormSMO->GetHInstance(),
            pFormSMO->GetHWND(), "Среднее время обслуживания запроса: 0");
    pLabelMidleTimeClinet->CreateLabel(10, 540, 400, 20);
}
//Создание полей ввода на форме
void InitEdit()
{
    pEditCountQuery = new Edit(pFormSMO->GetHInstance(), pFormSMO->GetHWND(),
            "30");
